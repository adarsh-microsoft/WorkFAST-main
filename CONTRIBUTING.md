...
│           └── architecture.md
├── .vscode/
│   └── mcp.template.json               ← MCP server registry template (committed)
└── .github/
    ...
    └── skills/                        ← Skill definitions (SKILL.md)
        ...
        ├── copilotstudio-devops/        ← Shared resource layer (config + modules)
        ├── copilotstudio-devops-*/SKILL.md ← Capability skills (6)
        ├── ado-board-hygiene/SKILL.md   ← ADO board hygiene skill
        ├── wiki-devops/SKILL.md         ← Wiki operations skill
        ├── memory/                      ← Memory skill (QMD search, daily logs, checkpoints)
        │   ├── SKILL.md
        │   ├── config/collections.yaml
        │   └── modules/checkpoint-protocol.md
...
argument-hint: '<usage hint for the WorkFast>'
...
| `user-invokable` | No | Set `false` for subagents only reachable via WorkFast |
| `argument-hint` | No | Usage hint displayed to the WorkFast |
| `agents` | No | List of subagents this agent can delegate to (WorkFast only) |
| `handoffs` | No | Quick-action buttons for the WorkFast |

### 3. Register the agent

- **In the WorkFast**: Add the agent to `WorkFast.agent.md`:
...
argument-hint: '<usage hint>'             # Optional. Helps WorkFast construct prompts.
...
handoffs:                                 # Optional. Quick-action buttons (WorkFast only).
  - label: <Button Label>
    agent: <target-agent>
...
2. **Test routing** — use prompts that should trigger your new skill/agent and verify the correct one activates:
   ```
   @WorkFast <prompt with your skill's trigger keywords>
   ```
...
5. **Test the full chain** — if your skill is invoked via the WorkFast, test the end-to-end flow (WorkFast → agent → skill → MCP tool → response).
...
| New agent | `@WorkFast <prompt that should route to new agent>` |
...
```
@WorkFast Run the evaluation suite
```

Or run a specific category:
```
@WorkFast Run the routing accuracy evaluations
```
...
| Change | Update |
| :--- | :--- |
| New WorkFast routing behavior | Add test scenarios to `eval-manifest.yaml`, rebaseline |
...
| Not updating the WorkFast | New agents must be registered in `WorkFast.agent.md` (frontmatter + body). |
...
| Skipping rebaseline after WorkFast changes | After modifying routing, scoring, or interaction behaviors, run the eval suite and update `baseline.yaml`. |

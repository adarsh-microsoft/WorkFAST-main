...
# ── 2. Check Azure CLI ────────────────────────────────────
Write-Host "[2/8] Checking Azure CLI..." -ForegroundColor Yellow
try {
...
Write-Host "[3/8] Checking VS Code extensions..." -ForegroundColor Yellow
...
Write-Host "[4/8] Pre-caching MCP server npm packages..." -ForegroundColor Yellow
...
Write-Host "[5/8] Configuring git hooks..." -ForegroundColor Yellow
...
Write-Host "[6/8] Verifying workspace MCP configuration..." -ForegroundColor Yellow

# $mcpJsonPath = Join-Path $PSScriptRoot ".vscode" "mcp.json"
$mcpJsonPath = "$PSScriptRoot\.vscode\mcp.json"
if (Test-Path $mcpJsonPath) {
    ...
    $mcpTemplatePath = "$PSScriptRoot\.vscode\mcp.template.json"
...
Write-Host "[7/8] Checking user context configuration..." -ForegroundColor Yellow
...
# ── 8. Setup QMD memory system ───────────────────────────
Write-Host "[8/8] Setting up QMD memory system..." -ForegroundColor Yellow

#$memorySetup = Join-Path $PSScriptRoot "scripts" "setup-memory.ps1"
$memorySetup =  "$PSScriptRoot\scripts\setup-memory.ps1"
if (Test-Path $memorySetup) {
    try {
        & $memorySetup
    } catch {
        Write-Host "  WARNING: Memory setup failed. You can run scripts/setup-memory.ps1 manually later." -ForegroundColor DarkYellow
        Write-Host "  Error: $($_.Exception.Message)" -ForegroundColor DarkYellow
    }
} else {
    Write-Host "  WARNING: scripts/setup-memory.ps1 not found" -ForegroundColor DarkYellow
}
...
Write-Host "  7. Invoke agents in Copilot Chat: @WorkFast, @chief-of-staff, @fabric-devops" -ForegroundColor White
Write-Host "  8. Memory search is available via the QMD MCP server (auto-starts in VS Code)" -ForegroundColor White
Write-Host ""

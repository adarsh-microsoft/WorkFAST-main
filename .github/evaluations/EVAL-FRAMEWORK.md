...
How to systematically test and measure the efficiency of the WorkFast agent and its subagents.

## Quick Start

```
@WorkFast Run evaluation suite from .github/evaluations/eval-manifest.yaml
```

Or run a single scenario:
```
@WorkFast Evaluate scenario RT-003
```

Or run a category:
```
@WorkFast Evaluate all routing scenarios
```
...
|-----------|-----------------|--------|
| **Routing Accuracy** | Did the WorkFast pick the right agent? | 22% |
| **Skill Activation** | Did the agent activate the correct skill? | 18% |
| **Prompt Quality** | Did the WorkFast construct a complete, well-structured prompt? | 15% |
...
| **Context Verification** | Did the WorkFast verify intent before acting on ambiguous/write requests? (v2) | 10% |
...
Ask the WorkFast to run the full evaluation:

```
@WorkFast Run the agent evaluation suite. For each scenario in 
...
```
@WorkFast Evaluate scenario RT-014: "Create tasks from my standup meeting today"
...
```
@WorkFast Evaluate all guardrail scenarios from the eval manifest.
```
...
```
@WorkFast Run regression: evaluate all scenarios and compare against baseline scores.
```

...
| 2026-02-18 | 4.0 | Refactored as WorkFast-managed subagent with constrained toolset and clear execution contracts. |
...
- `daily-status-mail-drafter` for drafting (NEVER sending) the Co-Marketing Daily Status Update using the canonical template — auto-fills today's date, queries today's ADO tasks for Adarsh and Prince, synthesizes one-liner Executive Summary bullets from each task's Discussion comments, and asks one confirming question on those bullets before creating the Outlook draft

> **Note:** ADO work item creation and updates are now handled by the `ado-devops` agent. If the user asks to create work items, inform the WorkFast to route to `ado-devops`.

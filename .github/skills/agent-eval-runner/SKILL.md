...
description: 'Run automated evaluation scenarios against the WorkFast and subagents. Dry-run classification tests that score routing accuracy, skill activation, prompt quality, and guardrail enforcement without executing actual subagent calls.'
---

# Agent Evaluation Runner

Execute evaluation scenarios from the eval manifest to measure WorkFast and agent efficiency. This skill runs **dry-run classification tests** — it processes prompts through the routing pipeline but does NOT make actual MCP tool calls or subagent delegations.
...
- `"Test the WorkFast routing"` or `"routing accuracy check"`
- `"Run regression after changes"` or `"check for regressions"`
- `"Evaluate scenario [ID]"` or `"test scenario RT-003"`
- `"Score the agents"` or `"how efficient is the WorkFast"`
...
| WorkFast.agent.md | Routing rules to test against |
...
#### 3a. Signal Extraction (from §3.1 of WorkFast)
...
Apply the WorkFast's scoring formula:
```
...
When the WorkFast evaluates itself, it must be honest about edge cases:
...
2. **Self-assessment bias** — the WorkFast grading itself may be optimistic; use human review for calibration

...
> **Cross-platform comparison:** To compare an on-prem SSAS/AAS tabular model against a Fabric semantic model, use the [ssas-connector](../ssas-connector/SKILL.md) skill instead. It handles schema extraction from both SSAS (via `Invoke-ASCmd`) and Fabric (via `powerbi-remote`), then produces a unified diff report using the same PASS/WARN/FAIL thresholds defined here.

...
| **SqlServer PowerShell module** | `Invoke-ASCmd` for SSAS/AAS connectivity (optional — needed only if using ssas-connector skill) | `Install-Module SqlServer -Scope CurrentUser` |
| **VS Build Tools** | Native compilation for QMD memory search (optional — needed only on Windows if using memory system) | `winget install Microsoft.VisualStudio.2022.BuildTools --override "--add Microsoft.VisualStudio.Workload.VCTools --quiet"` |
...
- **Sets up QMD memory system** — installs QMD, creates search collections, runs initial indexing

> **QMD on Windows:** QMD uses `better-sqlite3` which requires native C++ compilation. If `npm install -g @tobilu/qmd` fails with `gyp ERR`, install VS Build Tools first (see Prerequisites). The `setup.ps1` script handles this automatically.
>
> **QMD MCP server:** After setup, the QMD MCP server starts automatically when VS Code opens the workspace. It provides `qmd_query`, `qmd_get`, `qmd_multi_get`, and `qmd_status` tools for searching memory, daily logs, session checkpoints, and skill documentation.
...
### Step 5 (Optional): SSAS / AAS Server Catalog

If you connect to on-prem SQL Server Analysis Services (SSAS) or Azure Analysis Services (AAS) tabular models, edit `.github/skills/ssas-connector/config/ssas-catalog.yaml`:

```yaml
servers:
  - name: "MySSAS"
    host: "ssas-server.corp.contoso.com"        # On-prem SSAS server hostname
    database: "MyTabularModel"                   # Tabular model database name
    authType: "windows"                          # "windows" for integrated auth, "aad-token" for AAD
    notes: "Production tabular model for sales analytics"

  # Azure Analysis Services example:
  # - name: "MyAAS"
  #   host: "asazure://westus.asazure.windows.net/myserver"
  #   database: "AdventureWorks"
  #   authType: "aad-token"
  #   notes: "Azure AAS instance"
```

**Prerequisites for SSAS connectivity:**

| Requirement | Purpose | One-liner install |
|---|---|---|
| **SqlServer PowerShell module** | Provides `Invoke-ASCmd` for SSAS/AAS connections | `Install-Module SqlServer -Scope CurrentUser` |
| **Network access to SSAS** | Agent must reach the SSAS server | Verify with `Test-NetConnection <host> -Port 2383` |
| **Authentication** | Windows auth (domain-joined) or AAD token | Windows: automatic; AAD: `az login` covers it |

**Discovery prompt** — let Copilot auto-populate the catalog:
```
Connect to SSAS server <hostname> and discover all databases.
For each database, list the tables and measures. Then format the
output as entries for ssas-catalog.yaml.
```
...
|---|---|
| `@WorkFast Daily triage` | M365 tools + ADO + status email config |
| `@WorkFast What's the health of my DEV workspace?` | Fabric MCP + workspace catalog |
...
| `@WorkFast Run the evaluation suite` | Evaluation framework + all agent routing |
...
└── .github/
    ...
    └── skills/                        ← Skill definitions + config
        ...
        ├── memory/                    ← Memory skill (QMD search, checkpoints)
...
| **QMD** | stdio (node) | None (local index) | WorkFast + all agents (memory search) |
...
| Run agent evaluations | `@WorkFast Run the evaluation suite` |

...
└───────┬──────────┬──────────────┬──────────────────┬──────────────────┬──────────────────┘
        ...
        ▼          ▼              ▼                  ▼                  ▼
┌────────────┐ ┌────────────┐ ┌────────────────┐ ┌────────────────┐ ┌──────────────────┐ ┌────────────────┐
│ Chief of   │ │ ADO DevOps │ │ Fabric DevOps  │ │ Databricks     │ │ Copilot Studio   │ │ Wiki DevOps    │
│ Staff      │ │            │ │                │ │ DevOps         │ │ DevOps           │ │                │
│            │ │ 3 skills   │ │ 7 capability   │ │ 7 capability   │ │ 6 capability     │ │ 1 skill        │
│ 1 skill    │ │ ADO work   │ │ skills         │ │ skills         │ │ skills           │ │ Wiki + PBI     │
│ M365 only  │ │ items+compl│ │ Fabric + PBI   │ │ DB + UC + DBFS │ │ PAC CLI + API    │ │ screenshots    │
└─────┬──────┘ └─────┬──────┘ └───────┬────────┘ └───────┬────────┘ └────────┬─────────┘ └───────┬────────┘
      │               │               │                   │                   │                   │
      ▼               ▼               ▼                   ▼                   ▼                   ▼
┌──────────────────────────────────────────────────────────────────────────────────────────────────────────────┐
...
│  Fabric API · Power BI Remote · ADO · M365 (Mail/Calendar/Teams) · Context7 · NL2DAB · PAC CLI               │
└──────────────────────────────────────────────────────────────────────────────────────────────────────────────┘
```

| Layer | Role | Analogy |
| :--- | :--- | :--- |
| **WorkFast** | Single entrypoint; classifies intent, routes to specialists, synthesizes results | Front desk receptionist |
...
**Design principle:** Each layer has a single concern. The WorkFast triages. Agents strategize. Skills execute. MCP servers connect.

---

## How the WorkFast Routes Requests

The WorkFast is a **classifier + dispatcher**. It never calls domain tools directly — it uses `runSubagent` to delegate to specialists.
...
| `copilotstudio-devops` | copilot studio, agent, bot, topic, knowledge source, quarantine, direct line, evaluate agent, compare agents, agent drift, publish bot, pac copilot, copilot studio URL |

---

## WorkFast v2 Behaviors

The WorkFast includes several interaction and safety behaviors that govern how it processes requests beyond basic routing.

### Interaction Style

The WorkFast operates as a **collaborative, guidance-driven** system — not an autonomous executor. It presents plans before acting on non-trivial requests, surfaces assumptions explicitly, and seeks confirmation before write actions.
...
Before routing non-trivial requests, the WorkFast verifies understanding to prevent wasted subagent calls.
...
For lightweight, single-skill, read-only tasks (e.g., catalog lookups, agent registry queries), the WorkFast may execute directly without dispatching a subagent, reducing overhead.
...
After significant actions (multi-agent workflows, write actions), the WorkFast saves key results to `memory/session/` to preserve context across long conversations.

---

## Multi-Agent Composition Patterns

The WorkFast recognizes composite workflows and auto-sequences them:
...
│  │  ┌─────┴───────────────┴───────────────┴─────────────┴──────────────┴───────┐  │    │
│  │  │  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐                    │  │    │
│  │  │  │  Analyze      │  │  Release &   │  │  SSAS / AAS  │                    │  │    │
│  │  │  │  Lineage      │  │  Promote     │  │  Connector   │                    │  │    │
│  │  │  │  w=1.05       │  │  w=1.0       │  │  w=1.0       │                    │  │    │
│  │  │  └──────────────┘  └──────────────┘  └──────────────┘                    │  │    │
│  │  └──────────────────────────────────────────────────────────────────────────┘  │    │
│  └─────────────────────────────────────────────────────────────────────────────────┘    │
...
│  │  PowerShell:    Invoke-ASCmd (SqlServer module) — SSAS/AAS connectivity          │    │
...
When the Fabric DevOps agent receives a request from the WorkFast, it runs this internal pipeline:
...
| **ssas-connector** | powershell-ssas | — | powerbi-remote (for cross-platform) | context7 |
...
| `powershell-ssas` | SqlServer module (`Invoke-ASCmd`) | On-prem SSAS/AAS schema discovery and DAX query execution |
...
| **QMD** | `qmd` | stdio (node) | None (local index) | BM25 keyword search over memory, daily logs, session checkpoints, skill docs |
...
│                                                                                     │
│  WorkFast ──────────────────► runSubagent, todo, QMD (memory search)             │
│                                   QMD: query, get, multi_get, status                │
│                                                                                     │
...
```
  ...
  External API
       ...
       │
       ▼
  Response → MCP Server → Agent → WorkFast → User
```
...
├──────────────────────────────────────────────────────────────────┤
│  LAYER 4: WORKFAST PROMPT CONSTRUCTION                      │
│  WorkFast includes "Constraints: PROD is read-only"         │
...
| :--- | :--- | :--- | :--- |
| **WorkFast** | Cross-domain | Classify intent, route to specialists, compose multi-agent workflows, context verification, write gates | `runSubagent`, `todo` only |
...
| **Copilot Studio DevOps** | Microsoft Copilot Studio | Agent lifecycle — inventory, evaluate, validate, develop, release, security | PAC CLI, Dataverse API, Direct Line API, Power Platform Admin API |
...
3. **Users talk to the WorkFast.** It delegates to the right specialist.
...
### Copilot Studio DevOps Skills

| Skill | Weight | Purpose | Example Trigger |
| :--- | :--- | :--- | :--- |
| **Evaluate** | 1.1 | Conversational testing via Semantic Kernel / Direct Line | "Test my agent with these utterances" |
| **Inventory** | 1.0 | List agents, status, extract full YAML templates | "List all agents in this environment" |
| **Validate** | 0.95 | Cross-environment agent diff via template extraction | "Compare my DEV agent vs PROD" |
| **Develop** | 1.0 | Create/update topics, publish agents (non-PROD) | "Publish the agent in DEV" |
| **Release & Promote** | 1.0 | Solution export/import ALM lifecycle | "Promote agent solution to PROD" |
| **Security** | 1.0 | Quarantine, audit, DLP compliance | "Quarantine this non-compliant agent" |
...
└────────────────────────────────────────┬────────────────────────────────────────────────┘
                                         ...
                                         ▼
┌─────────────────────────────────────────────────────────────────────────────────────────┐
│  2. WORKFAST — INTENT CLASSIFICATION                                                │
│                                                                                         │
│  Signal extraction:                                                                     │
...
└────────────────────────────────────────┬────────────────────────────────────────────────┘
                                         ...
                                         ▼
┌─────────────────────────────────────────────────────────────────────────────────────────┐
│  3. WORKFAST — PROMPT CONSTRUCTION (Step A)                                         │
│                                                                                         │
...
└────────────────────────────────────────┬────────────────────────────────────────────────┘
                                         ...
                                         ▼
┌─────────────────────────────────────────────────────────────────────────────────────────┐
│  5. WORKFAST — PROMPT CONSTRUCTION (Step B, chained)                                │
│                                                                                         │
│  runSubagent(agentName="fabric-devops", prompt="""                                      │
│    ## Objective                                                                         │
...
| Add a new agent | Create `.github/agents/<name>.agent.md` + register in `WorkFast.agent.md` |
| Add ambiguity rules | Edit `skills/fabric-devops/config/intent-router.yaml` `ambiguityRules` section |
| Run agent evaluations | `@WorkFast Run the evaluation suite` — tests 54 scenarios across 12 categories |

...
> Deep-dive into the internals of the Fabric DevOps agent: intent routing, skill activation, execution engines, MCP tool topology, authorization, shared resources, safety guardrails, and how it all connects to the overall WorkFast architecture.
...
- [How the WorkFast Invokes Fabric DevOps](#how-the-WorkFast-invokes-fabric-devops)
  - [WorkFast → Agent Handoff](#WorkFast--agent-handoff)
...
The Fabric DevOps agent sits at **Layer 2 (Domain Agent)** in the three-layer architecture. The WorkFast routes to it; it dispatches to capability skills; skills call MCP tools.
...
**Key distinction:** The WorkFast decides *which agent*. The agent decides *which skill*. The skill decides *which procedure steps and tool calls*.
...
When the Fabric DevOps agent receives a delegated request (from the WorkFast via `runSubagent`), it runs this deterministic 8-step pipeline:

```
  Request from WorkFast
  ─────────────────────────
...
```
  ...
  │     External API → MCP Server → Agent → WorkFast → User                    │
  └─────────────────────────────────────────────────────────────────────────────────┘
```
...
║  .github/agents/WorkFast.agent.md                                        ║
║                                                                              ║
...
## How the WorkFast Invokes Fabric DevOps

### WorkFast → Agent Handoff

The WorkFast never calls Fabric tools directly. It uses `runSubagent` with a structured prompt:
...
The WorkFast follows these enrichment rules when building prompts for Fabric DevOps:
...
Some requests require multiple skills to execute sequentially within the Fabric DevOps agent. The WorkFast handles this by making multiple `runSubagent` calls:
